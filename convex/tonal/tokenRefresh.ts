...
export const refreshExpiringTokens = internalAction({
  handler: async (ctx) => {
    ...
    while (true) {
      ...
      } = await ctx.runQuery(internal.tonal.tokenQueries.getExpiringTokensPage, {
        beforeTimestamp: twoHoursFromNow,
        paginationOpts: { cursor, numItems: EXPIRING_TOKENS_PAGE_SIZE },
      });
      ...
      for (const profile of page.page) {
        try {
          ...
          const lockAcquired = await ctx.runMutation(
            internal.userProfiles.acquireTokenRefreshLock,
          ...
          const refreshResult = await refreshTonalToken(refreshToken);

          const encryptedToken = await encryptToken(refreshResult.idToken, keyHex);
          const encryptedRefresh = refreshResult.refreshToken
            ? await encryptToken(refreshResult.refreshToken, keyHex)
            : undefined;
          ...
          await ctx.runMutation(internal.userProfiles.updateTonalToken, {
            ...
            tonalTokenExpiresAt: refreshResult.expiresAt,
          });
