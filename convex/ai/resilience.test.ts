...
describe("finalizeCode derivation (TONALCOACH-1C)", () => {
  it("transient → kind code; non-Error → unknown_error", () => {
    const t = new Error("This model is currently experiencing high demand.");
    expect(classifyTransientError(t) ?? t.name).toBe("provider_overload");
    expect(classifyTransientError("oops") ?? "unknown_error").toBe("unknown_error");
  });
});
