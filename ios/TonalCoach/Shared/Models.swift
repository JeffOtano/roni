...
// MARK: - Week Plan Presentation (from coach chat messages)

/// Weekly training plan generated by the AI coach.
/// Matches `WeekPlanPresentation` in `convex/ai/schemas.ts`.
struct WeekPlanPresentation: Decodable {
...
struct WeekPlanDay: Decodable, Identifiable {
    let dayName: String
    let sessionType: String
    let targetMuscles: String
    let durationMinutes: Int
    let exercises: [WeekPlanExercise]

    var id: String { dayName }
}

struct WeekPlanExercise: Decodable, Identifiable {
    let name: String
    let sets: Int
    let reps: Int?
    let duration: Int?
